...
| `proposal_not_sendable` | Proposal already sent/confirmed, currently sending, or wrong state | Reload; inspect audit; do not force duplicate send |
...
| Stale send/booking manual review | Process stopped during an external provider action | Check Meta/Google first; never repeat until provider state is known |
