...
class SchedulingService:
        ...
        for entry in payload.get("entry", []):
            for change in entry.get("changes", []):
                ...
                for message in value.get("messages", []):
                    ...
                    active = self.store.latest_sent_proposal(self.settings.workspace_id, sender)
                    if active:
                    ...
                    reply = self._proposal_reply(intent.language, slots, token)
    ...
    def send_proposal(self, proposal_id: int, actor: str = "operator") -> dict:
        ...
        try:
            ...
            self.store.record_outbound(self.settings.workspace_id, proposal["sender"])
    ...
    def confirm_public(self, token: str, slot_index: int, consent: bool) -> int:
        ...
        self.store.update_contact(
            ...
            email=proposal.get("email"), timezone_name=self.settings.timezone, language=proposal.get("language", "en"),
    ...
    def run_due_jobs(self, limit: int = 20) -> dict[str, int]:
        totals = {"claimed": 0, "sent": 0, "manual_required": 0, "failed": 0}
        if self.is_paused():
            return totals
        for job in self.store.claim_due_jobs(self.settings.workspace_id, utc_now().isoformat(), limit):
            ...
            try:
                ...
                local = datetime.fromisoformat(appointment["start_at"]).astimezone(ZoneInfo(appointment["timezone"]))
                ...
                self.store.record_outbound(self.settings.workspace_id, appointment["sender"])
    ...
    def _proposal_reply(self, language: str, slots: list[datetime], token: str) -> str:
        rendered = [self._format_slot(slot, language, index + 1) for index, slot in enumerate(slots)]
        link = f"{self.settings.public_base_url}/book/{token}"
        if language == "nl":
            return "Ik kan deze momenten aanbieden:\n" + "\n".join(rendered) + f"\n\nAntwoord met 1, 2 of 3, of kies via {link}"
        return "I can offer these times:\n" + "\n".join(rendered) + f"\n\nReply with 1, 2 or 3, or choose at {link}"
    ...
    def _send_booking_confirmation(self, appointment: dict) -> None:
        ...
        if appointment["language"] == "nl":
            text = f"Bevestigd: {appointment['title']} op {local:%d-%m-%Y} om {local:%H:%M} ({appointment['timezone']})."
        else:
            text = f"Confirmed: {appointment['title']} on {local:%d-%m-%Y} at {local:%H:%M} ({appointment['timezone']})."
        self.whatsapp.send_reply(appointment["sender"], text)
        self.store.record_outbound(self.settings.workspace_id, appointment["sender"])

    def _enforce_rate_limit(self, sender: str) -> None:
        if not self.store.outbound_allowed(self.settings.workspace_id, sender, self.settings.max_outbound_per_contact_per_hour):
